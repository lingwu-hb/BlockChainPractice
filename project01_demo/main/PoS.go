...
var users []User = make([]User, 100)
var userNum int

//用户结构体
type User struct {
	coinNumber int64
	address    string //用于身份验证
	_durtime   time.Time
	weight     int64 //用户权益值
}
...
func Initialize() {
	...
	for ; i <= n; i++ {
		...
		users[i].coinNumber = coinNumber
		users[i].address = address
		users[i]._durtime = time.Now()
		users[i].ComputeWeight()
	}
	userNum = i - 1
}
...
var BlockList []Block = make([]Block, 100)
...
func main() {
	...
	for {
		...
		cut++
		validator := ReturnIndex()
		GenerateNextBlock(data, BlockList[cut-1], cut, users[validator].address)
	}
	Print_()
}
